...
public partial class NativeMethods
{
    ...
    [DllImport("iphlpapi.dll", CharSet = CharSet.Auto, SetLastError = true)]
    public static extern ERROR ConvertInterfaceIndexToLuid(uint InterfaceIndex, out ulong InterfaceLuid);

    [DllImport("iphlpapi.dll", CharSet = CharSet.Auto, SetLastError = true)]
    public static extern ERROR ConvertInterfaceLuidToGuid(in ulong InterfaceLuid, out Guid InterfaceGuid);

    [DllImport("iphlpapi.dll", CharSet = CharSet.Auto, SetLastError = true)]
    public static extern ERROR ConvertInterfaceAliasToLuid(string InterfaceAlias, out ulong InterfaceLuid);
    ...
    public static List<Route> GetRoutes(FAMILY family)
    {
        ...
        if (Environment.OSVersion.Version.CompareTo(new Version("6.0")) < 0)
        {
            ...
            for (int i = 0; i < table.dwNumEntries; i++)
            {
                ...
                route.Destination = new IPAddress(row.dwForwardDest);
                route.Prefix = new IPAddress(row.dwForwardMask).ToString();
                route.Gateway = new IPAddress(row.dwForwardNextHop);
                route.InterfaceIndex = row.dwForwardIfIndex;
        ...
        else
        {
            ...
            for (int i = 0; i < table.NumEntries; i++)
            {
                ...
                route.InterfaceIndex = row.InterfaceIndex;
                route.Age = row.Age;
                route.Metric = (ushort)row.Metric;
                if (row.DestinationPrefix.Prefix.Ipv4.sin_family == FAMILY.AF_INET)
                {
                    route.Destination = new IPAddress(row.DestinationPrefix.Prefix.Ipv4.sin_addr.S_addr);
                    route.Gateway = new IPAddress(row.NextHop.Ipv4.sin_addr.S_addr);
                    IP.ValidateIPv4Mask(ref route.Prefix);
                    route.IPVersion = 4;
                }
                else
                {
                    route.Destination = new IPAddress(row.DestinationPrefix.Prefix.Ipv6.sin6_addr.Byte);
                    route.Gateway = new IPAddress(row.NextHop.Ipv6.sin6_addr.Byte);
                    route.IPVersion = 6;
                }
                routes.Add(route);
    ...
    public static void AddRoute(string destination, string prefix, string gateway, uint interfaceIndex, ushort metric, MIB_IPFORWARD_TYPE type = MIB_IPFORWARD_TYPE.MIB_IPROUTE_TYPE_INDIRECT, NL_ROUTE_PROTOCOL protocol = NL_ROUTE_PROTOCOL.MIB_IPPROTO_NETMGMT)
    {
        if (Environment.OSVersion.Version.CompareTo(new Version("6.0")) < 0)
        {
        ...
        else
        {
            ...
            route2.InterfaceIndex = interfaceIndex;
    ...
    /// <param name="gateway"></param>
    /// <param name="interfaceIndex"></param>
    public static void DeleteRoute(string destination, string prefix, string gateway, uint interfaceIndex)
    {
        if (Environment.OSVersion.Version.CompareTo(new Version("6.0")) < 0)
        {
            MIB_IPFORWARDROW route = new MIB_IPFORWARDROW();
    ...
    public static List<Adapter> GetAdapters(FAMILY family)
    {
        ...
        else
        {
            ...
            if (error == ERROR.ERROR_BUFFER_OVERFLOW)
            {
                ...
                error = GetAdaptersAddresses((uint)family, (uint)FLAGS.GAA_FLAG_INCLUDE_PREFIX, IntPtr.Zero, pAdapterAddresses, ref outBufLen);
                if (error == ERROR.ERROR_SUCCESS)
                {
                    ...
                    while (currPtr != IntPtr.Zero)
                    {
                        ...
                        adapter.Description = adapterAddress.Description;
